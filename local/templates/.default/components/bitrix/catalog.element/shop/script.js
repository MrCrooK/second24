...
$(document).ready(function(){
	...
	/* Фото в быстром просмотре */
	  $( ".big_photo" ).bind( "mouseenter", function( event ) {
		$(".zoom_item").html('<img class="biiiigphoto" src="'+$( ".big_photo" ).attr('src')+'">');
		$(".zoom_item").show();
	  });
	  ...
	  $( ".big_photo" ).bind( "mousemove", function( event ) {
		...
		$(".zoom_item img").css({left: '-'+(Xinner*scale-350)+'px', top: '-'+(Yinner*scale-150)+'px', });
	  });
	/* Конец Фото в быстром просмотре */

	/* Выбор превьюшки */
	$('.changeimage').eq(0).addClass('selected');
